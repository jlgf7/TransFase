...
#Cálculo da Vv
Vv_calc <- 1 - exp(-(4/3)*N*(tempo^3))

#Cálculo da Sv
Sv_calc <- 12*N*(tempo^2)*exp(-(4/3)*N*(tempo^3))

#Cálculo de Caminho Microestrutural
SVV <- (3*(36*N)^(1/3))*(1-Vv_calc)*((log(1/(1-Vv_calc)))^(2/3))
...
legend(x = 110, y = 0.1, legend=c("Simulated", "Calculated"), lty=c(NA,1), lwd=1:1, bty="n", pch=c(1,NA),  col=c("red","blue"), 
       pt.cex = 1.5)
dev.off()
...
legend(x = 55, y = 0.0047, legend=c("Simulated", "Calculated"), lty=c(NA,1), col=c("red","blue"), lwd=1:1, bty="n", pch=c(1,NA), pt.cex = 1.5)
dev.off()
...
par( mar = c(5,5,4,2))
grafVv <- plot(tempo,SVR, type = "l", xlab = "Time", ylab = expression(bold(S[Vr])), col="blue", cex.lab= 1.4, font.lab = 2)
...
par( mar = c(5,5,4,2))
grafVv <- plot(tempo, Gteo, type = "l", xlab = "Time", ylab = "G", col = "blue",  cex.lab= 1.4, font.lab = 2)
grafVv <- points(tempored, Gred, col = "red", cex = 1.5)
legend(x = 111, y = 0.23, legend=c("Simulated", "Calculated"), lty=c(NA,1), col=c("red","blue"), lwd=1:1, bty="n", pch=c(1,NA), pt.cex = 1.5)
dev.off()
